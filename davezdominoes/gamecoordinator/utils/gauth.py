...
try:
    ...
    _grassmere = "1f0e643a93c4da6ba929"
...
def getUri(secret, login):
     issuer = "DavezDominoes"
     # see http://code.google.com/p/google-authenticator/wiki/KeyUriFormat
     uri = "otpauth://totp/{issuer}:{login}?"\
...
def getQRCode(gauthKey, user):
    ...
    data = BytesIO()
    qrcode.make(getUri(secret, user.login), box_size=6).save(data)
